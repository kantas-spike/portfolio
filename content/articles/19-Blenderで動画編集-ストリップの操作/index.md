...
ショートカットキー **Kキー**(Split) や **Shift+Kキー**(Hold Split)を使って Split した場合、
Playheadのどちら側に マウスカーソルがあるかによって、ストリップ分割後のストリップの選択状態が変わります。

| Split or Hold Split 時のカーソルの位置 | 説明                                                                                                                             |
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| マウスカーソルがPlayheadの右側         | 分割後の右側のストリップは **アクティブで未選択** になります。逆に、分割後の左のストリップは **非アクティブで選択** になります。 |
| マウスカーソルがPlayheadの左側         | 分割後の左側のストリップは **アクティブで未選択** になります。逆に、分割後の右のストリップは **非アクティブで選択** になります。 |
...
| **Ctrlキー**           | ストリップが **左ドラッグ中** や **グラブ状態** の時に、**Ctrlキー** を押し続けると、スナップの有無が切り替わります。デフォルトでスナップを無効にしている場合は、**Ctrlキー** を押している間だけスナップが有効になります。 |
...
| 右方向に指定フレーム分移動 | 数値                   | グラブ状態の時に、数値を入力すると、右方向に入力した数値のフレーム分移動する。**-10** のように負の値を入力すると左方向に指定したフレーム分移動する。また、四則演算などの計算式を入力し、計算結果分の移動可能です。[^1] |
