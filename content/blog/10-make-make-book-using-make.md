...
そして、勉強の成果として、[PDFで公開されているMake本をマージする | kanta's spike](https://kantas-spike.github.io/portfolio/blog/2023/02/09-merge-make-book/)で作成したシェルスクリプトを`makefile`に置き換えたいと思います。
...
シェルスクリプトをもとに、『Make本』のマージ版PDFを作成するための、依存関係を整理します。

| target | prereq | メモ |
|---|---|---|
| make_merged.pdf | マージ済みPDF[^4] | [make_book_pdf/03_add_toc.sh](https://github.com/kantas-spike/make_book_pdf/blob/main/old_tool/03_add_toc.sh)から整理|
|| 目次情報ファイル[^3] ||
| build/_make.pdf | ダウンロードした章別のPDF群 | [make_book_pdf/02_merge_pdf.sh](https://github.com/kantas-spike/make_book_pdf/blob/main/old_tool/02_merge_pdf.sh)から整理 |
||マージ版PDFの説明PDF[^2]||
| ダウンロードした章別のPDF群 | 章別PDFのURL一覧[^5]| [make_book_pdf/01_download_pdf.sh](https://github.com/kantas-spike/make_book_pdf/blob/main/old_tool/01_download_pdf.sh)から整理 |
||PDFダウンロード用ディレクトリ[^6]||
|マージ版PDFの説明PDF[^2]| マージ版PDFの説明原稿[^7] | `cp`で作成 |
|PDFダウンロード用ディレクトリ[^6]||`mkdir -p`で作成|
|目次情報ファイル[^3]||手動で作成|
|章別PDFのURL一覧[^5]||手動で作成|
|マージ版PDFの説明原稿[^7]||手動で作成|
...
シェルスクリプトと組合せる場合、改行方法や、変数名を`$$変数名`に変更する必要があるなど、ややこしいですね。

作成済の他ツールのMakefileを修正しながら、`make`になれていきたいと思います。
